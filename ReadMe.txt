...
(2023/07/25): Complete rewrite of the character creation system. Charactres now initialize as a blank character and sheets are loaded as character.load(). Application global variables are now created in character for character.show(). Fixed bug for characters with no skills, or no saving throws. Character creation and editing are now a shared interface. Manually editing .5e files should no longer be necessary.